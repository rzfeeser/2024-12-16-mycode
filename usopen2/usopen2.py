#!/usr/bin/env python3
## USOpen Tournament Switch Checker -- v2 -- 2024.12.17
...
- Apply new configuration (import netmiko) # THIS IS NEW!
...
def main():
    ...
    file_location = input("Host file locattion [Press ENTER for default: 'host_list.csv']\n>") or "host_list.csv"
    ...
    if ynchk.lower() in ["y", "yes"]:
        ...
        conf_ip = input("\nHostname or ip of the device to be configured? \n>")

        # loop across our list of switch dict
        for switch_info in entry:
            if conf_ip in switch_info.values():   # check to see if the switch to update is within the dict we are iterating on
                if bootstrapper.bootstrapper(switch_info['ip'], switch_info['driver'], switch_info['username'], switch_info['password'], conf_loc):
